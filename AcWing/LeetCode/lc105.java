...
class Solution {    //递归建树 + 模板题 + 数学推导
    Map<Integer, Integer> pos = new HashMap<>();    //中序遍历中映射位置哈希表，优化查找的值的位置的时间复杂度
    ...
    TreeNode build(int[] preorder, int[] inorder, int pl, int pr, int il, int ir) {
        ...
        root.left = build(preorder, inorder, pl + 1, pl + 1 + (k - 1 - il), il, k - 1);   //递归build左子树
        root.right = build(preorder, inorder, pl + 1 + (k - 1 - il) + 1, pr, k + 1, ir);  //递归build右子树
        return root;
    }
}
